...
//        rearrangeGreaterThanZeroAtLeftSide6(arr);
        rearrangeZeroAtRightSideButMaintainOrderOfOtherElement(arr);
    ...
    private static void rearrangeZeroAtRightSideButMaintainOrderOfOtherElement(int[] arr) {
        int i =0 , j=i+1;
        long l = Arrays.stream(arr).filter(ele -> ele == 0).count();
        while(i!=l){
            if(arr[i]==0){
                i++;
                j=i+1;
