...
public class FindPair {
    public static void main(String[] args) {
        int[] numbers = {3, 1, 4, 1, 5}; int k = 2; //2

//        int[] numbers = {1, 3, 1, 5, 4}; int k = 0;

//          int[] numbers = {1,3,1,5,4};
//          int k = 0;

        int pairCount = findPairs2(numbers, k);
        System.out.println("PairCount ====>" + pairCount);
    }

    private static int findPairs2(int[] numbers, int k) {
    ...
    private static int findPairs(int[] numbers, int k) {
        ...
        for (int num: numbers){
           /*
            Integer addedValue = map.putIfAbsent(num,1);
            if(addedValue!=null){
